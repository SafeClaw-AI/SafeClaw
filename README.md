...
- `service-status`：查看当前 service 队列 / worker / effect / probe / heartbeat / coordination 摘要，并显式展示 recent task 的 `scope` / `write` / `doctor_bypass`、当前 `permission_tier` / `permission_policy` / `permission_reason`、`lease_age_ms` / `lease_freshness`、same-scope peer 可见化（`scope_peer_count` / `scope_active_peer_count` / `scope_active_peer_task_id`）、active lease 的等待时间、`next_action` 决策提示、可直接复制的 `next_command`、简短的 `next_reason`、当前阻断项 `next_blocker`、协调态 `coordination_status` / `coordination_reason` / `coordination_summary`，以及一行可抄录的 `next_summary`
