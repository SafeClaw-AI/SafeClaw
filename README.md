...
如需显式确认当前动作在本地离线 MVP 中仍被允许，可先执行 `safeclaw.cmd preflight --action service-run`；常见 wrapper / session 动作会自动从 remembered session / workspace / 默认 output 推断权限上下文；若要显式覆盖，可追加 `--scope demo.workspace`、`--write`、`--doctor-bypass`；若要让脚本在 `confirm/deny` 时直接 fail-closed，可再加 `--enforce-permission`。
...
- `service-status`：查看当前 service 队列 / worker / effect / probe / heartbeat / coordination 摘要，并显式展示 recent task 的 `scope` / `write` / `doctor_bypass`、当前 `permission_tier` / `permission_policy` / `permission_reason`、`lease_age_ms` / `lease_freshness`、same-scope peer / scope-quarantine 可见化（`scope_peer_count` / `scope_active_peer_count` / `scope_active_peer_task_id` / `scope_quarantine_active` / `scope_quarantine_source` / `scope_quarantine_task_id` / `scope_quarantine_count`）、active lease 的等待时间、`next_action` 决策提示、建议先关注的 `next_task_id`、可直接复制的 `next_command`、简短的 `next_reason`、当前阻断项 `next_blocker`（含 `scope_quarantine`）、协调态 `coordination_status` / `coordination_reason` / `coordination_summary`，以及一行可抄录的 `next_summary`
