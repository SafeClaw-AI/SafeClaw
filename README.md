...
如需显式确认当前动作在本地离线 MVP 中仍被允许，可先执行 `safeclaw.cmd preflight --action service-run`；若要提前预检 scope 权限，可追加 `--scope demo.workspace`、`--write`、`--doctor-bypass`。
...
- `preflight`：显式预检某个目标动作在当前本地离线 MVP 中是否仍被允许；已知 wrapper / session 动作可离线运行，未知动作默认从严拒绝；如传入 `--scope` / `--write` / `--doctor-bypass`，还会同步返回 `permission_tier` / `permission_policy` / `permission_reason` 供执行前判断
