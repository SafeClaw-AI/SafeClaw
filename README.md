...
- `service-status`：查看当前 service 队列 / worker / effect / probe 摘要，并显式展示 recent task 的 `scope` / `write` / `doctor_bypass`、最新 lease 新鲜度、`next_action` 决策提示，以及可直接复制的 `next_command`
