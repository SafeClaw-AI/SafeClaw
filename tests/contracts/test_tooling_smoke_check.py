...
class ToolingSmokeCheckTest(unittest.TestCase):
    ...
    def test_collect_errors_uses_wrapper_service_recover_invalid_limit_json_helper(
        self,
    ) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
    ...
    def test_append_wrapper_service_recover_invalid_limit_json_errors_keeps_labels(
        self,
    ) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        ...
        self.assertIn("assert_command_json_error(", helper_block)
        self.assertIn('"mvp-wrapper-service-recover-invalid-limit-json"', helper_block)
        self.assertIn('"service-recover"', helper_block)
        self.assertIn("PYTHON", helper_block)
        self.assertIn('"tools/mvp/safeclaw_mvp.py"', helper_block)
        self.assertIn('expected_error_message_substring="invalid --limit: bogus"', helper_block)
    ...
    def test_append_wrapper_service_demo_text_errors_keeps_labels(self) -> None:
        helper_block = (
            ...
            / "tools"
            / "checks"
            / "tooling_smoke_service_demo_text.py"
        ).read_text(encoding="utf-8")
        self.assertIn("subprocess_module.run(", helper_block)
    ...
    def test_collect_errors_uses_wrapper_service_demo_no_tool_path_json_helper(
        self,
    ) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
    ...
    def test_append_wrapper_service_demo_invalid_json_errors_keeps_labels(
        ...
        helper_block = (
            ...
            / "tools"
            / "checks"
            / "tooling_smoke_service_demo_invalid_json.py"
        ).read_text(encoding="utf-8")
    ...
    def test_collect_errors_uses_wrapper_cmd_run_json_helper(self) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        )
        normalized_source = normalize_source_whitespace(source)
        self.assertIn("append_wrapper_cmd_run_json_errors(errors)", normalized_source)

    def test_append_wrapper_cmd_run_json_errors_keeps_labels(self) -> None:
        helper_block = (
            REPO_ROOT
            / "tools"
            / "checks"
            / "tooling_smoke_cmd_run_json.py"
        ).read_text(encoding="utf-8")
        self.assertIn("assert_command_json_result(", helper_block)
        self.assertIn("assert_run_json_result(", helper_block)
        self.assertIn('"mvp-wrapper-cmd-run-json"', helper_block)
        self.assertIn('"task-wrapper-cmd-space"', helper_block)
        self.assertIn('"cmd"', helper_block)
    ...
    def test_collect_errors_uses_wrapper_forget_success_helper(self) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        )
        normalized_source = normalize_source_whitespace(source)
        self.assertIn(
            "append_wrapper_forget_success_errors(errors)", normalized_source
        )

    def test_append_wrapper_forget_success_errors_keeps_boundary(self) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        )
        helper_block = source.split(
        ...
        self.assertIn("mvp-wrapper-forget 输出缺少会话清空标记", helper_block)
        self.assertIn("mvp-wrapper-forget-json 输出不符合预期", helper_block)
        self.assertIn("mvp-wrapper-forget-json 输出缺少 reason=none", helper_block)
        self.assertIn("mvp-wrapper-session-after-forget 输出缺少 none/path", helper_block)
        self.assertIn('"forget"', helper_block)
        self.assertIn('"session"', helper_block)
        self.assertNotIn("mvp-wrapper-cmd-forget-json missing session path", helper_block)
        self.assertNotIn(
            "mvp-wrapper-restore-after-cmd-forget missing task-wrapper-b",
            helper_block,
        )
    ...
    def test_collect_errors_uses_wrapper_demo_invalid_argument_helper(
        self,
    ) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        ...
        normalized_source = normalize_source_whitespace(source)
        self.assertIn(
            "append_wrapper_demo_invalid_argument_errors( errors,",
            normalized_source,
        )

    def test_append_wrapper_demo_invalid_argument_errors_keeps_boundary(
        self,
    ) -> None:
        source = (
            REPO_ROOT
            / "tools"
            / "checks"
            / "tooling_smoke_wrapper_demo_invalid_argument.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"mvp-wrapper-cmd-demo-fail-json"', source)
        self.assertIn('"mvp-wrapper-ps1-demo-fail-json"', source)
        self.assertIn('"mvp-wrapper-demo-fail-json"', source)
        self.assertNotIn('"mvp-wrapper-demo-underlying-fail-json"', source)
        self.assertNotIn('"mvp-wrapper-demo-enforced-json"', source)
        self.assertNotIn('"mvp-wrapper-recover-demo-fail-json"', source)

    def test_append_wrapper_demo_invalid_argument_errors_keeps_failure_contract(
        self,
    ) -> None:
        source = (
            REPO_ROOT
        ...
        self.assertIn('expected_failed_step="run"', source)
        self.assertIn('expected_code="invalid-argument"', source)
        self.assertIn('_DEMO_TASK_ID = "task-wrapper-demo-json"', source)
        self.assertIn(
            '"mvp-wrapper-demo-fail-json remembered_session 缺少 task-wrapper-demo-json"',
            source,
        )
    ...
    def test_collect_errors_uses_wrapper_recover_demo_success_helper(self) -> None:
        ...
        normalized_source = normalize_source_whitespace(source)
        self.assertIn(
            "append_wrapper_recover_demo_success_errors( errors,",
            normalized_source,
        )

    def test_append_wrapper_recover_demo_success_errors_keeps_boundary(self) -> None:
        source = (
            REPO_ROOT
            / "tools"
            / "checks"
            / "tooling_smoke_wrapper_recover_demo_success.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"mvp-wrapper-recover-demo"', source)
        self.assertIn('"mvp-wrapper-recover-demo-json"', source)
        self.assertNotIn('"mvp-wrapper-recover-demo-preflight-json"', source)
        self.assertNotIn('"mvp-wrapper-recover-demo-fail-json"', source)
        self.assertNotIn('"mvp-wrapper-demo-json"', source)
        self.assertNotIn('"mvp-wrapper-retry-demo-json"', source)

    def test_append_wrapper_recover_demo_success_errors_keeps_session_links(
        self,
    ) -> None:
        source = (
            REPO_ROOT
    ...
    def test_collect_errors_uses_wrapper_recover_demo_failure_helper(self) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        )
        normalized_source = normalize_source_whitespace(source)
        self.assertIn(
            "append_wrapper_recover_demo_failure_errors( errors,",
            normalized_source,
        )

    def test_append_wrapper_recover_demo_failure_errors_keeps_boundary(
        self,
    ) -> None:
        source = (
            REPO_ROOT
            / "tools"
            / "checks"
            / "tooling_smoke_wrapper_recover_demo_failure.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"mvp-wrapper-recover-demo-fail"', source)
        self.assertIn('"mvp-wrapper-cmd-recover-demo-fail-json"', source)
        self.assertIn('"mvp-wrapper-ps1-recover-demo-fail-json"', source)
        self.assertIn('"mvp-wrapper-recover-demo-fail-json"', source)
        self.assertNotIn('"mvp-wrapper-recover-demo-preflight-json"', source)
        self.assertNotIn('"mvp-wrapper-demo-fail-json"', source)
        self.assertNotIn('"mvp-wrapper-retry-demo-fail-json"', source)

    def test_append_wrapper_recover_demo_failure_errors_keeps_failure_contract(
        self,
    ) -> None:
        source = (
            REPO_ROOT
        ...
        self.assertIn('expected_failed_step="seed-crash"', source)
        self.assertIn(
            '"mvp-wrapper-recover-demo-fail 输出缺少组合动作失败承接提示"',
            source,
        )
        self.assertIn(
            '"mvp-wrapper-recover-demo-fail-json remembered_session 缺少 task-wrapper-recover-demo-json"',
            source,
        )
        self.assertIn("reject_legacy_session=True", source)
    ...
    def test_collect_errors_uses_wrapper_retry_demo_preflight_success_helper(
        self,
    ) -> None:
        source = (REPO_ROOT / "tools" / "checks" / "check_tooling_smoke.py").read_text(
            encoding="utf-8"
        ...
        normalized_source = normalize_source_whitespace(source)
        self.assertIn(
            "append_wrapper_retry_demo_preflight_success_errors( errors,",
            normalized_source,
        )

    def test_append_wrapper_retry_demo_preflight_success_errors_keeps_boundary(
        self,
    ) -> None:
        source = (
            REPO_ROOT
            / "tools"
            / "checks"
            / "tooling_smoke_wrapper_retry_demo_preflight_success.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"mvp-wrapper-retry-demo-preflight-json"', source)
        self.assertIn('"task-wrapper-retry-demo-json"', source)
        self.assertNotIn('"mvp-wrapper-retry-demo-fail-json"', source)
        self.assertNotIn('"mvp-wrapper-demo-preflight-json"', source)
        self.assertNotIn('"mvp-wrapper-recover-demo-preflight-json"', source)

    def test_append_wrapper_retry_demo_preflight_success_errors_keeps_preflight_contract(
        self,
    ) -> None:
        source = (
            REPO_ROOT
        ...
        self.assertIn('"mvp-wrapper-retry-demo-preflight-json preflight"', source)
        self.assertIn('expected_requested_action="retry-demo"', source)
        self.assertIn(
