...
最后更新时间：2026-04-06 08:04:41 +08:00
...
- 最近三次提交：`7937fa4 feat: lock personal deploy governance closeout`、`f6ca6a1 test: lock selfcheck governance closeout`、`80fe243 docs: lock boundary governance closeout`。
- 下一步：若开启新一轮迭代，先刷新本摘要与分组表，再新增批次清单。

### 维护备注（不参与公开基线）
- 当前本地仍在继续收口 `M2-320 ~ M2-329` 这批自检治理改动。
- 当前待提交内容以各轮次“待提交内容”与工作区实际改动为准，不回写到上方公开基线摘要。

## 当前提交/归档收口表（2026-04-04）

| 批次 | 当前状态 | 已验结果 | 提交边界 |
| --- | --- | --- | --- |
| 边界治理 | 已提交 | `80fe243` | 不独立开发丞相模式/大都督模式，只保留后期外部拼接融合边界 |
| 自检治理 | 已提交 | `f6ca6a1` | `selfcheck.py` 继续作为唯一总验收入口，不绕行 |
| 个人部署链 | 已提交 | `7937fa4` | 只服务 `archive-note -> status -> undo`，回滚只切 release 指针 |
...
## 流水
### 轮次 M2-319
- 完成时间：2026-04-06 04:37:34 +0800
- 完成内容：继续收 `check_mvp_operator_flow.py` 里首轮独占 DB 的重复清场；在脚本开头已经统一 `reset_operator_flow_state()` 的前提下，把 `service-run`、`seed-failed`、`seed-crash`、`seed-hibernated` 等首轮建库步骤上的冗余 `--reset` 去掉；同时在 `tests/contracts/test_mvp_operator_flow_check.py` 补上源码合同，锁住“首轮独占 DB 不再重复 reset”这条边界。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 tools/checks/check_tooling_smoke.py`
- 结果备注：这轮主要是去冗余，不是大提速；`verify --json` 仍约 `10.3s`，其内部 `operator-flow` 结束点约在 `+9.3s`；本轮真实 `tooling-smoke` 为 `210.1s`，没有比上一轮 `195.8s` 更快。结论很直接：首轮 `--reset` 清理已经更干净，但当前总耗时大头不在这里。
- 待提交内容：`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-318
- 完成时间：2026-04-06 04:20:28 +0800
- 完成内容：继续收 `check_mvp_operator_flow.py` 里的重复组合动作；把 `operator-flow/service-resume` 从 `resume -> service-status -> report` 三步，收成 `resume -> service-status` 两步；这条 `service-resume --report` 成功合同继续留在 `check_tooling_smoke.py` 的独立护栏里，不在 `operator-flow` 里重复跑。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_mvp_operator_flow_check -v`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 tools/checks/check_tooling_smoke.py`
- 结果备注：真实 `python -X utf8 tools/mvp/safeclaw_mvp.py verify --json` 继续从约 `10.2s` 降到约 `10.0s`；真实 `tooling-smoke` 已从 `204.8s` 再降到 `195.8s`；其中 `[tooling-smoke 060]` 的 `safeclaw.cmd verify --json` 已从 `10.2s` 再降到 `9.4s`，`[tooling-smoke 341]` 的 wrapper `verify --json` 继续保持约 `0.2s`。
- 待提交内容：`tools/mvp/safeclaw_mvp.py`、`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-317
- 完成时间：2026-04-06 04:10:31 +0800
- 完成内容：继续收 `safeclaw_mvp.py` 的运行时重活；把主 wrapper 的 `run_sqlite_example_capture(...)` 改成像 personal MVP 一样，优先走预编译 `target\debug\examples\safeclaw_mvp_entry.exe`，只有找不到 exe 时才回退 `cargo run`；同时补 `tests/contracts/test_tooling_smoke_check.py`，锁住“`safeclaw_mvp_entry` 可走 exe、其它 example 仍走 cargo”这条边界。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_resolve_safeclaw_mvp_runtime_command_prefers_prebuilt_example tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_run_sqlite_example_capture_uses_prebuilt_example_when_available tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_run_sqlite_example_capture_keeps_other_examples_on_cargo -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py`
- 结果备注：真实 `python -X utf8 tools/mvp/safeclaw_mvp.py verify --json` 已从 `15.0s` 继续降到 `10.2s`；真实 `tooling-smoke` 已从 `259.5s` 继续降到 `204.8s`；其中 `[tooling-smoke 060]` 的 `safeclaw.cmd verify --json` 已从 `15.6s` 降到 `10.2s`，`[tooling-smoke 341]` 的 wrapper `verify --json` 继续保持约 `0.3s`。
- 待提交内容：`tools/mvp/safeclaw_mvp.py`、`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-316
- 完成时间：2026-04-06 03:52:59 +0800
- 完成内容：继续收 `check_mvp_operator_flow.py` 的重复 Python 启动开销；把 `load_json()` / `run_json()` 从“每步都再起一个 `safeclaw_mvp.py` 子进程”改成“同进程直接调用 `safeclaw_mvp.py main(...)` 并抓取原样 JSON 输出”；同时补 `tests/contracts/test_mvp_operator_flow_check.py`，锁住这条同进程入口，不让后续再悄悄退回外层 Python 子进程。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_mvp_operator_flow_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py`
- 结果备注：真实 `python -X utf8 tools/mvp/safeclaw_mvp.py verify --json` 已从约 `25.9s` 降到 `15.0s`；真实 `tooling-smoke` 已从 `264.2s` 再降到 `259.5s`；其中 `[tooling-smoke 060]` 的 `safeclaw.cmd verify --json` 已从 `22.4s` 降到 `15.6s`，`[tooling-smoke 341]` 的 wrapper `verify --json` 继续保持约 `0.2s`。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-315
- 完成时间：2026-04-06 03:34:35 +0800
- 完成内容：继续收 `check_tooling_smoke.py` 的重复 `verify` 重活；把 `tools\mvp\safeclaw_mvp.cmd verify --json` 改成“真入口 + 临时 `sitecustomize.py` 假 operator-flow”的轻量 JSON 透传校验；同时修掉上一轮误插到 `wrapper_restore_after_ps1_retry_a` 的 `env=build_smoke_pythonpath_env(Path(verify_mock_dir))`，让真实 `tooling-smoke` 重新回到全绿。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`
- 结果备注：本轮真实 `tooling-smoke` 已从 `322.7s` 继续降到 `264.2s`；其中 `[tooling-smoke 060]` 的 `safeclaw.cmd verify --json` 仍保留真实成功态校验，当前约 `22.4s`；`[tooling-smoke 341]` 的 `tools\mvp\safeclaw_mvp.cmd verify --json` 已降到约 `0.2s`；真实 `python -X utf8 tools/mvp/safeclaw_mvp.py verify --json` 仍通过，当前约 `25.9s`，并直接回放 `40` 个阶段点。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-314
- 完成时间：2026-04-06 03:08:21 +0800
- 完成内容：继续收 `check_tooling_smoke.py` 的重复 `verify` 重活；把 wrapper 成功态从 `tools\mvp\safeclaw_mvp.cmd verify` plain 文本重活，改成文档一致的 `verify --json`；同时把 smoke 里的直连 Python `verify --json` 改成“独立 Python 进程内走 `run_verify(['--json'])`，但把内部 `subprocess.run` 替成已知成功结果”的轻量 JSON 出口校验，不再重复跑完整 operator flow；并补 `tests/contracts/test_tooling_smoke_check.py`，锁住 wrapper `verify --json` 边界，以及 `run_verify` 的 plain/json 两条输出语义。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`
- 结果备注：本轮真实 `tooling-smoke` 已从 `364.6s` 降到 `322.7s`；其中 `[tooling-smoke 342]` 已从一整次约 `28s` 的直连 Python `verify --json`，变成约 `0.1s` 的 JSON 出口校验；真实 `python -X utf8 tools/mvp/safeclaw_mvp.py verify --json` 仍通过，当前约 `24.2s`。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-313
- 完成时间：2026-04-06 02:40:46 +0800
- 完成内容：继续收 `check_tooling_smoke.py` 的重复 `verify` 重活；把 `safeclaw.ps1 verify --json` 成功态改成 `verify --bogus --json` 轻量透传检查，只保留 `safeclaw.cmd` 与直连 wrapper 的成功态 `verify`；同时补 `tests/contracts/test_tooling_smoke_check.py`，锁住“root ps1 走轻量错误透传、root cmd 继续保留成功态”这条边界。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`
- 结果备注：本轮真实 `tooling-smoke` 中 `safeclaw.ps1 verify` 已从成功态重活变成 `[tooling-smoke 059]` 的 0.5 秒错误透传检查；`safeclaw.cmd verify --json` 仍保留 `[tooling-smoke 060]` 的真实成功态校验。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-312
- 完成时间：2026-04-06 02:00:19 +0800
- 完成内容：继续拆 `verify` 黑箱；`tools/checks/check_mvp_operator_flow.py` 现在会对 `reset-state / run_json / wait_for_session` 打印 `[operator-flow NNN] start/done` 阶段心跳，`verify --json` 已能直接回放 `50` 个阶段点；同时新增 `tests/contracts/test_mvp_operator_flow_check.py`，锁住计数器重置、`run_json` 心跳与 `wait_for_session` 心跳。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_mvp_operator_flow_check tests.contracts.test_tooling_smoke_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 tools/checks/check_tooling_smoke.py`
- 待提交内容：`tools/checks/check_mvp_operator_flow.py`、`tests/contracts/test_mvp_operator_flow_check.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/check_tooling_smoke.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-311
- 完成时间：2026-04-06 01:38:55 +0800
- 完成内容：继续收 `check_tooling_smoke.py`；现在不只会打印 `start/done` 心跳，还会在父进程被外部打断时，尽快终止当前子命令并退出，避免继续占 `target/mvp/.wrapper-check.lock`。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard -v`；真实启动 `python -X utf8 tools/checks/check_tooling_smoke.py` 后强杀外层 shell，3 秒内确认 `lock-missing`，且最近 3 分钟内无残留 `python/powershell` 进程。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`

### 轮次 M2-310
- 完成时间：2026-04-06 01:27:24 +0800
- 完成内容：给 `tools/checks/check_tooling_smoke.py` 补上统一 `start/done` 进度心跳；现在每条 `subprocess.run` 都会输出序号、命令摘要和耗时，长跑时能直接看到卡在第几步，不再只剩“看起来像卡住”。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard -v`；试跑 `python -X utf8 tools/checks/check_tooling_smoke.py`，已看到 `tooling-smoke 001..020` 的连续心跳输出。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`tools/checks/README.md`、`开发计划.md`、`MVP_PROGRESS.md`、`PUSH_LOG.md`
...
### Round NC
- 完成时间：2026-03-30 05:11:15 +0800
- 本轮完成：新增 `_parse_python_text_for_reference_check()` 与 `PythonTextParseResult`，把“`path.as_posix()` + `ast.parse()` + `SyntaxError` 转人话”收成单一真源；现在空异常、缺少上下文、绑定未使用、静默降级四条 Python 检查链都统一复用同一份 parse 结果。
- 验证：python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py、python -m unittest tests.contracts.test_reference_redlines_check -v、python tools/checks/check_reference_redlines.py、python tools/checks/check_ledger_alignment.py、git diff --check。
- 提交推送：本轮提交信息拟为 refactor: unify python parse helper；最终 hash 以当前 HEAD 为准。
### Round ND
- 完成时间：2026-03-30 05:16:05 +0800
- 本轮完成：新增 `_collect_python_reference_redline_errors()`，把三条顶层 Python 文件扫描器里重复的“筛 `.py` / 读文件 / 转相对路径 / 调 collector”收成单一真源；现在缺少上下文、绑定未使用、静默降级三条顶层扫描器都统一复用这一入口。
- 验证：python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py、python -m unittest tests.contracts.test_reference_redlines_check -v、python tools/checks/check_reference_redlines.py、python tools/checks/check_ledger_alignment.py、git diff --check。
- 提交推送：本轮提交信息拟为 refactor: unify python file scan helper；最终 hash 以当前 HEAD 为准。
### Round NE
- 完成时间：2026-03-30 05:20:54 +0800
...
### Round NG
- 完成时间：2026-03-30 06:03:02 +0800
- 本轮完成：把 `TypeError` 纳入 `SILENT_FALLBACK_EXCEPTION_TYPE_ORDER`，继续扩大 direct `None/False` 静默降级红线；当前全仓 `TypeError` catch 命中为 0，本轮属于零旧债扩面。
- 验证：python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py、python -m unittest tests.contracts.test_reference_redlines_check -v、python tools/checks/check_reference_redlines.py、python tools/checks/check_ledger_alignment.py、git diff --check。
- 提交推送：本轮提交信息拟为 feat: block typeerror silent fallback；最终 hash 以当前 HEAD 为准。
### Round NH
- 完成时间：2026-03-30 06:10:55 +0800
- 本轮完成：把 direct 静默降级从 `None/False` 扩到空字符串 / 空容器，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return ''`、`except TypeError: return []/{}/()` 这类 direct silent fallback；当前全仓该形态仍为 0 命中，本轮属于零旧债扩面。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block empty silent fallback`；最终 hash 以当前 HEAD 为准。
### Round NI
- 完成时间：2026-03-30 06:23:54 +0800
- 本轮完成：补齐“空容器”语义漏口，`tools/checks/check_reference_redlines.py` 现在会把 `except ValueError: return set()` 与 `except TypeError: return frozenset()` 也视为 direct silent fallback；当前全仓该形态仍为 0 命中，本轮属于零旧债扩面。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block empty container call fallback`；最终 hash 以当前 HEAD 为准。
### Round NJ
- 完成时间：2026-03-30 06:29:30 +0800
- 本轮完成：把 `str()` / `list()` / `dict()` / `tuple()` 这组无参空值构造调用也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return str()`、`except TypeError: return list()/dict()/tuple()` 这类降级；当前全仓该形态仍为 0 命中，本轮属于零旧债扩面。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block empty constructor call fallback`；最终 hash 以当前 HEAD 为准。
### Round NK
- 完成时间：2026-03-30 06:35:14 +0800
...
### Round NN
- 完成时间：2026-03-30 06:50:58 +0800
- 本轮完成：把 `fallback: T = 空值; return fallback` 这类 `AnnAssign` 两步静默降级也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except OSError: fallback: object = None; return fallback` 与 `except TypeError: fallback: list[str] = []; return fallback`；当前全仓该形态仍为 0 命中，本轮属于零旧债扩面。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block annassign return fallback`；最终 hash 以当前 HEAD 为准。
### Round NO
- 完成时间：2026-03-30 07:05:11 +0800
...
### Round NQ
- 完成时间：2026-03-30 07:25:28 +0800
- 本轮完成：把 `bool(False)`、`dict([])`、`bytes([])`、`list(())` 这类静态单参空值构造 alias 也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return dict([])`、`except TypeError: return bytes([])` 与 `except OSError: fallback = bool(False); return fallback` 这类绕行；当前全仓该形态仍为 0 命中，本轮属于零旧债扩面。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block static single-arg fallback`；最终 hash 以当前 HEAD 为准。
### Round NR
- 完成时间：2026-03-30 07:32:07 +0800
...
### Round NY
- 完成时间：2026-03-30 08:34:22 +0800
- 本轮完成：把 `return constructor(ifexp)` 包装也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: fallback = []; return list(fallback if True else [1])`、`except TypeError: fallback = bool(False); return bool(True if False else fallback)` 与 `except OSError: empty = "" + ""; alias = empty; return str(alias if True else "fallback")` 这类参数层静态分支绕行；定向扫描 return call + IfExp silent fallback 为 `NO_RETURN_CALL_IFEXP_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block return call ifexp fallback`；最终 hash 以当前 HEAD 为准。
### Round NZ
- 完成时间：2026-03-30 08:39:38 +0800
- 本轮完成：把 `return constructor(boolop)` 包装也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: fallback = []; return list(fallback or [])`、`except TypeError: fallback = bool(False); return bool(True and fallback)` 与 `except OSError: empty = "" + ""; alias = empty; return str(alias or "")` 这类参数层短路表达式绕行；定向扫描 return call + BoolOp silent fallback 为 `NO_RETURN_CALL_BOOLOP_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block return call boolop fallback`；最终 hash 以当前 HEAD 为准。
### Round OA
- 完成时间：2026-03-30 08:49:46 +0800
- 本轮完成：把 `return constructor(unary/compare)` 包装也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: fallback = []; return bool(fallback == [1])`、`except TypeError: fallback = bool(False); return bool(not not fallback)` 与 `except OSError: empty = "" + ""; alias = empty; return bool(alias != "")` 这类参数层比较 / `not` 表达式绕行；定向扫描 return call + UnaryOp/Compare silent fallback 为 `NO_RETURN_CALL_UNARY_OR_COMPARE_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block return call unary compare fallback`；最终 hash 以当前 HEAD 为准。
### Round OB
- 完成时间：2026-03-30 08:56:32 +0800
...
### Round OH
- 完成时间：2026-03-30 10:10:29 +0800
- 本轮完成：把 `copy()` 零参方法也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return [].copy()`、`except TypeError: mapping = {}; return mapping.copy()` 与 `except OSError: values = []; return list(values.copy())` 这类 empty-preserving 方法调用绕行；定向扫描 except handler 内的 direct / constructor-wrapped `copy()` return 为 `NO_RETURN_COPY_METHOD_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block copy method fallback`；最终 hash 以当前 HEAD 为准。

### Round OI
- 完成时间：2026-03-30 10:19:50 +0800
...
### Round OK
- 完成时间：2026-03-30 10:48:34 +0800
- 本轮完成：把 `isalnum/isalpha/isdecimal/isdigit/isidentifier/islower/isnumeric/isspace/istitle/isupper` 这组 zero-arg text predicate method 也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return ''.isalnum()`、`except TypeError: payload = b""; return payload.isdigit()` 与 `except OSError: name = "" + ""; return bool(name.isidentifier())` 这类 `False` 型文本谓词方法绕行；定向扫描 except handler 内的 text predicate method return 为 `NO_RETURN_TEXT_PREDICATE_METHOD_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block text predicate fallback`；最终 hash 以当前 HEAD 为准。

### Round OL
- 完成时间：2026-03-30 11:00:50 +0800
- 本轮完成：把 `capitalize/swapcase/title/expandtabs` 这组 zero-arg text transform method 也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return ''.capitalize()`、`except TypeError: payload = b""; return payload.expandtabs()` 与 `except OSError: payload = bytearray(); return bytearray(payload.title())` 这类 empty-preserving 文本变换方法绕行；定向扫描 except handler 内的 text transform method return 为 `NO_RETURN_TEXT_TRANSFORM_METHOD_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block text transform fallback`；最终 hash 以当前 HEAD 为准。

### Round OM
- 完成时间：2026-03-30 11:12:13 +0800
...
### Round OQ
- 完成时间：2026-03-30 11:47:25 +0800
- 本轮完成：把 `update/difference_update/intersection_update` 这组 zero-arg collection update mutator method 也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return {}.update()`、`except TypeError: payload = set(); return payload.difference_update()` 与 `except OSError: payload = set(); return bool(payload.intersection_update())` 这类更新型 mutator 绕行；定向扫描 except handler 内的 update mutator method return 为 `NO_RETURN_UPDATE_MUTATOR_METHOD_HITS`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block update mutator fallback`；最终 hash 以当前 HEAD 为准。

### Round OR
- 完成时间：2026-03-30 11:54:28 +0800
...
### Round OT
- 完成时间：2026-03-30 18:10:33 +0800
- 本轮完成：把空 `GeneratorExp` 被 `list/tuple/dict/set/frozenset` 消费的场景也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return list(item for item in ())`、`except TypeError: payload = set(); return tuple(item for item in payload)` 与 `except OSError: pairs = []; return dict((key, value) for key, value in pairs)` 这类 generator expression 空容器绕行；实现方式是新增 generator expression 空值哨兵，只在 consuming constructor 消费时折叠为静默降级真值，不误伤 direct generator return。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block empty generator fallback`；最终 hash 以当前 HEAD 为准。

### Round OU
- 完成时间：2026-03-30 18:19:55 +0800
- 本轮完成：把空 `GeneratorExp` 被 `bytes/bytearray` 消费的场景也纳入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return bytes(item for item in ())`、`except TypeError: payload = []; return bytearray(item for item in payload)` 与 `except OSError: payload = []; return bytes(source=(item for item in payload))` 这类字节构造器绕行；实现方式仍沿用 generator 哨兵，只在 consuming constructor 消费空 generator expression 时折叠为 `b""/bytearray()`。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block bytes generator fallback`；最终 hash 以当前 HEAD 为准。

### Round OV
- 完成时间：2026-03-30 18:27:50 +0800
...
- 本轮完成：沿通用空迭代器哨兵把 `iter()` 接入 direct silent fallback，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: return list(iter(()))`、`except TypeError: payload = []; return tuple(iter(payload))` 与 `except OSError: payload = []; return bytes(iter(payload))` 这类 `iter()` 空迭代器绕行。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: block empty iter fallback`；最终 hash 以当前 HEAD 为准。
### Round OX
- 完成时间：2026-03-30 18:42:57 +0800
- 本轮完成：把通用空迭代器哨兵打通到 handler 赋值链，`tools/checks/check_reference_redlines.py` 现在会阻断 `except ValueError: payload = []; items = iter(payload); return list(items)`、`except TypeError: payload = []; items = zip(payload, [1]); return tuple(items)` 与 `except OSError: payload = []; items = (item for item in payload); return set(items)` 这类 alias 传播绕行。
- 验证：`python -m py_compile tools/checks/check_reference_redlines.py tests/contracts/test_reference_redlines_check.py`、`python -m unittest tests.contracts.test_reference_redlines_check -v`、`python tools/checks/check_reference_redlines.py`、`python tools/checks/check_ledger_alignment.py`、`git diff --check`。
- 提交推送：本轮提交信息拟为 `feat: track empty iterator aliases`；最终 hash 以当前 HEAD 为准。
### Round OY
- 完成时间：2026-03-30 18:53:09 +0800
...
- Done: Connected empty `any()` evaluation to the silent-fallback truth source in `tools/checks/check_reference_redlines.py`, blocking `except ValueError: return any([])`, `except TypeError: payload = []; return any(payload)`, and `except OSError: payload = []; flag = any(payload); return bool(flag)`.
...
- Commit: planned message `feat: block empty any fallback`; final hash follows HEAD.
...
- Done: Connected empty `sum()` evaluation to the silent-fallback truth source in `tools/checks/check_reference_redlines.py`, letting existing bool/compare logic fail-close `except ValueError: return bool(sum([]))`, `except TypeError: payload = []; return bool(sum(payload))`, and `except OSError: payload = []; return sum(payload) > 0`.
...
- Commit: planned message `feat: block empty sum fallback`; final hash follows HEAD.
...
- Done: Extended empty `replace()` evaluation to the three-argument `replace(old, new, count)` shape in `tools/checks/check_reference_redlines.py`, fail-closing `except ValueError: return "".replace("x", "y", 1)`, `except TypeError: payload = b""; return payload.replace(b"x", b"y", 1)`, and `except OSError: payload = b""; cleaned = payload.replace(b"x", b"y", 1); return bytearray(cleaned)`.
...
- Commit: planned message `feat: block empty replace count fallback`; final hash follows HEAD.

### Round PY
- Time: 2026-03-31 12:38:59 +0800
- Done: Extended empty `replace()` evaluation to the keyword-count shape `replace(old, new, count=...)` in `tools/checks/check_reference_redlines.py`, fail-closing `except ValueError: return "".replace("x", "y", count=1)`, `except TypeError: payload = b""; return payload.replace(b"x", b"y", count=1)`, and `except OSError: payload = b""; cleaned = payload.replace(b"x", b"y", count=1); return bytearray(cleaned)`.
...
- Commit: planned message `feat: block empty replace keyword count fallback`; final hash follows HEAD.
...
- Done: Extended parameterized empty `encode()/decode()` evaluation in `tools/checks/check_reference_redlines.py` to cover the keyword `encoding=` shape, fail-closing `except ValueError: return "".encode(encoding="utf-8")`, `except TypeError: payload = b""; return payload.decode(encoding="utf-8")`, and `except OSError: payload = bytearray(); text = payload.decode(encoding="utf-8"); return bytearray(text.encode(encoding="utf-8"))`.
...
- Commit: planned message `feat: block empty codec keyword encoding fallback`; final hash follows HEAD.

### Round QC
- Time: 2026-03-31 18:36:13 +0800
- Done: Added empty `format(...)` evaluation in `tools/checks/check_reference_redlines.py`, fail-closing `except ValueError: return "".format(1)`, `except TypeError: template = ""; return template.format(name=1)`, and `except OSError: template = ""; text = template.format(1, name=2); return bytearray(text.encode())`.
...
- Commit: planned message `feat: block empty format fallback`; final hash follows HEAD.

### Round QD
- Time: 2026-03-31 18:54:44 +0800
- Done: Extended the parameterized empty `encode()/decode()` helper in `tools/checks/check_reference_redlines.py` to cover keyword `errors=` and combined `encoding+errors` shapes, fail-closing `except ValueError: return "".encode(errors="ignore")`, `except TypeError: payload = b""; return payload.decode(errors="ignore")`, and `except OSError: payload = bytearray(); text = payload.decode(encoding="utf-8", errors="ignore"); return bytearray(text.encode(encoding="utf-8", errors="ignore"))`.
...
- Commit: planned message `feat: block empty codec errors fallback`; final hash follows HEAD.

### Round QE
- Time: 2026-03-31 19:23:57 +0800
- Done: Added empty `format_map(...)` evaluation in `tools/checks/check_reference_redlines.py` for both static expression evaluation and known-name runtime resolution, and widened known-name assignment tracking to accept statically resolvable values so alias mappings can participate; this now fail-closes `except ValueError: return "".format_map({})`, `except TypeError: template = ""; mapping = {"name": 1}; return template.format_map(mapping)`, and `except OSError: template = ""; mapping = {"name": 2}; text = template.format_map(mapping); return bytearray(text.encode())`.
...
- Commit: planned message `feat: block empty format_map fallback`; final hash follows HEAD.
...
### 轮次 M2-320
- 完成时间：2026-04-06 05:09:23 +0800
- 完成内容：继续只收自检治理；`tools/checks/check_tooling_smoke.py` 新增 `write_smoke_demo_sitecustomize()`、`build_smoke_demo_pythonpath_env()` 与 `should_use_smoke_demo_sitecustomize()`，现在只对直连 Python 的 `demo / recover-demo / retry-demo` 成功路径自动注入轻量假底层，保留失败路径与公开 `verify` 成功态的真实语义；`tests/contracts/test_tooling_smoke_check.py` 已补 helper 与选择器合同，防止后续回退。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 tools/checks/check_tooling_smoke.py`（`192.6s`） 、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-321
- 完成时间：2026-04-06 05:37:15 +08:00
- 完成内容：继续只收自检治理；先给 `tools/checks/check_tooling_smoke.py` 增加 wrapper shell 成功路径选择器，尝试把 `tools\mvp\safeclaw_mvp.cmd/.ps1` 的 `service-run / service-retry / service-recover` 一并接入统一 `sitecustomize` 轻量入口；整跑 `tooling_smoke` 后发现 `retry/recover` 的 `service_status` JSON 会丢 `queue.expired=0`，于是同轮立刻回缩，只保留已验证稳定的 `service-run` 成功路径；`tests/contracts/test_tooling_smoke_check.py` 已补对应合同。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`python -X utf8 tools/checks/check_tooling_smoke.py`（首次失败后回缩重跑通过，最终 `199.2s`）、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-322
- 完成时间：2026-04-06 05:56:37 +08:00
- 完成内容：继续只收自检治理；先把 `write_smoke_demo_sitecustomize()` 的 `seed-*` 假输出补到至少含 `task/db/output`，并短暂尝试把直连 Python `seed-failed/seed-crash/seed-hibernated` 成功路径接入统一 `sitecustomize` 轻量入口；整跑 `tooling_smoke` 后发现这类 stub 只会伪造 stdout，不会落真实会话/队列状态，导致后续 `resume/retry/recover/service-*` 大面积断链，于是同轮立刻回缩，撤掉 seed 运行路径接线，只保留更完整的 `seed stdout` 假输出与对应合同。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`（先红后绿）、`python -X utf8 tools/checks/check_tooling_smoke.py`（中途失败一次后定位回缩，最终 `179.7s` 通过）、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-323
- 完成时间：2026-04-06 06:18:56 +08:00
- 完成内容：继续只收自检治理；把根入口 `safeclaw.cmd verify --json` 成功路径改成“临时目录下写入 verify 专用 `sitecustomize.py`，只在该次探针里通过 `PYTHONPATH` 注入”的轻量探针；`tests/contracts/test_tooling_smoke_check.py` 同步补上 `test_root_cmd_verify_uses_success_json_guard_with_sitecustomize`，锁住成功态只在探针边界命中轻量假底层，不改公开 `verify` 成功 JSON 语义。
- 验证内容：`python -X utf8 -m py_compile tools/checks/check_tooling_smoke.py tests/contracts/test_tooling_smoke_check.py`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`（约 `9.6s`）、`python -X utf8 tools/checks/check_tooling_smoke.py`（顺序重跑通过，约 `171.0s`）、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-324
- 完成时间：2026-04-06 06:36:52 +08:00
- 完成内容：继续只收自检治理；扩展 `write_smoke_demo_sitecustomize()`，让它除 `safeclaw_mvp_entry` 外也能拦截 `worker_service_governance_demo`，并为 `service-demo` 返回固定治理快照；`tests/contracts/test_tooling_smoke_check.py` 同步补上 stub 输出合同，以及 python/cmd 成功路径 selector 合同，锁住 `service-demo` 成功态只在探针边界命中轻量假底层，`--bogus` 失败路径保持真语义。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_write_smoke_demo_sitecustomize_creates_combo_stub tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_should_use_smoke_demo_sitecustomize_matches_combo_success_paths tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_should_use_smoke_wrapper_service_sitecustomize_matches_shell_success_paths -v`、`python -X utf8 -m py_compile tools/checks/check_tooling_smoke.py tests/contracts/test_tooling_smoke_check.py`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/mvp/safeclaw_mvp.py verify --json`（约 `9.6s`）、`python -X utf8 tools/checks/check_tooling_smoke.py`（顺序重跑通过，约 `169.1s`）、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-326
- 完成时间：2026-04-06 07:11:57 +08:00
- 完成内容：继续只收自检治理；先对 `M2-325` 做顺序日志对比，确认“只给 PowerShell 补 `-NoProfile/-NonInteractive`”没有稳定收益后当轮回滚；随后在 `tools/checks/check_tooling_smoke.py` 新增 `write_smoke_report_sitecustomize()`、`build_smoke_report_pythonpath_env()` 与 `should_use_smoke_wrapper_report_sitecustomize()`，只对 `powershell.exe -File tools\mvp\safeclaw_mvp.ps1 report --db ... --task-id ... --json` 这组显式成功路径注入临时 `sitecustomize` 轻量探针；`tests/contracts/test_tooling_smoke_check.py` 同步补上 report stub 与 selector 合同，锁住 crash / failed 两类 report 关键字不漂移，`report --bogus --json` 与无 session 失败路径保持真语义。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_write_smoke_report_sitecustomize_creates_report_stub tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_wrapper_shell_report_success_paths_use_report_stub_selector tests.contracts.test_tooling_smoke_check.ToolingSmokeCheckTest.test_should_use_smoke_wrapper_report_sitecustomize_matches_shell_success_paths -v`、`python -X utf8 -m py_compile tools/checks/check_tooling_smoke.py tests/contracts/test_tooling_smoke_check.py`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py *> tmp/tooling-smoke-20260406/tooling-smoke-m2-326-seq.log`（顺序重跑通过，约 `164.9s`）、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-328
- 完成时间：2026-04-06 07:47:49 +08:00
- 完成内容：继续只收自检治理；在 `tools/checks/check_tooling_smoke.py` 新增 `_SMOKE_WRAPPER_SERVICE_REPORT_STUB_TASK_IDS` 与 `should_use_smoke_wrapper_service_report_sitecustomize()`，只对 `powershell.exe -File tools\mvp\safeclaw_mvp.ps1 service-retry/service-recover/service-resume/service-reconcile ... --report --json` 这 4 组成功路径注入 report 专用 `sitecustomize`；这刀只缩掉组合动作里的内层 `report` 重活，不伪造 `service-status` / queue / session 的真实状态；`tests/contracts/test_tooling_smoke_check.py` 同步补上对应 selector 合同。
- 验证内容：`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`、`python -X utf8 -m py_compile tools/checks/check_tooling_smoke.py tests/contracts/test_tooling_smoke_check.py`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py *> tmp/tooling-smoke-20260406/tooling-smoke-m2-328-trial.log`（顺序重跑通过，约 `162.3s`）、`git diff --check`
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`

### 轮次 M2-329
- 完成时间：2026-04-06 08:04:41 +08:00
- 完成内容：继续只收自检治理；在 `tools/checks/check_tooling_smoke.py` 新增 `_SMOKE_ROOT_PS1_SERVICE_REPORT_STUB_TASK_IDS` 与 `should_use_smoke_root_ps1_service_report_sitecustomize()`，只对 `powershell.exe -ExecutionPolicy Bypass -File safeclaw.ps1 service-run/service-retry/service-recover/service-resume/service-reconcile ... --report --json` 这 5 组成功路径注入同一套 report 专用 `sitecustomize`；这刀仍只缩掉 root 组合动作里的内层 `report` 重活，不伪造 `service-status` / queue / session 的真实状态；`tests/contracts/test_tooling_smoke_check.py` 同步补上 root selector 合同，并额外锁住 `--preflight` 失败路径不得误命中。
- 验证内容：`python -X utf8 -m py_compile tools/checks/check_tooling_smoke.py tests/contracts/test_tooling_smoke_check.py`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check -v`、`python -X utf8 -m unittest tests.contracts.test_tooling_smoke_check tests.contracts.test_mvp_operator_flow_check tests.contracts.test_selfcheck tests.contracts.test_mvp_state_guard tests.contracts.test_public_docs_check -v`、`python -X utf8 tools/checks/check_tooling_smoke.py *> tmp/tooling-smoke-20260406/tooling-smoke-m2-329-trial.log`（顺序重跑通过，约 `160.7s`）、`git diff --check`
- 结果备注：这轮不是大提速，而是把 root `safeclaw.ps1` 的同类重活再切小一层；按顺序日志看，`service-retry/service-recover/service-reconcile` 各再降约 `0.1s`，`service-resume` 约持平，`service-run` 四舍五入后仍在 `0.9~1.0s` 这一档；整套 `tooling_smoke` 仍从上一轮约 `162.3s` 继续降到约 `160.7s`，所以这刀保留记账，但下一刀不该继续围着 `service-*-report` 打转。
- 待提交内容：`tools/checks/check_tooling_smoke.py`、`tests/contracts/test_tooling_smoke_check.py`、`MVP_PROGRESS.md`、`PUSH_LOG.md`、`开发计划.md`
