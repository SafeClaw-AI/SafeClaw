...
pub const CORE_SPEC_BINDINGS: [SpecBinding; 7] = [
    ...
    SpecBinding { spec_path: "specs/schemas/task_concurrency.json", module_path: "safeclaw_core::scheduler", stage: ImplementationStage::RuntimeSlice, next_step: "已接 SQLite orchestrator、真实 worker loop、scope 协调、batch short-circuit、batch conflict、batch release 与 resume conflict 示例；后续扩 sidecar queue / 多 worker 协调" },
