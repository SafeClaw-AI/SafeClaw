...
pub const CORE_SPEC_BINDINGS: [SpecBinding; 7] = [
    ...
    SpecBinding { spec_path: "specs/schemas/task_concurrency.json", module_path: "safeclaw_core::scheduler", stage: ImplementationStage::RuntimeSlice, next_step: "已接 SQLite orchestrator、真实 worker loop、worker loop dispatch 入口、覆盖 fresh / retry / resume / probe 四分流的 dispatch demo，以及 dispatch until empty 批量入口与 dispatch batch demo；orchestrator / worker_loop write under reads、orchestrator / worker_loop serial writes under reads、orchestrator / worker_loop persisted uncertain probe recovery、orchestrator / worker_loop read fanout、scope 协调、batch short-circuit、batch conflict、batch release、resume conflict、resume missing runtime、resume release、retry conflict、retry missing runtime、retry pre-exec spawn recovery 与 retry release 示例；后续扩 sidecar queue / 多 worker 协调" },
