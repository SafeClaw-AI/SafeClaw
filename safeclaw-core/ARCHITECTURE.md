...
- 当前已具备单 worker 最小闭环：`claim -> execute -> crash -> persist -> restore -> probe -> complete` 的真实回归测试，并提供 `worker_loop_batch_failure_demo`、`worker_loop_batch_conflict_demo` 在内的 `worker_loop_*`、`worker_loop_scope_*` 与 `full_lifecycle_demo` 示例。
