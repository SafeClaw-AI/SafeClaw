...
- `tools/checks/`：协议层门禁检查（含 reference 红线、结构性债务与绕过白名单门禁）
- `tools/lint/`：稳定命名检查
...
- 在这条 ledger policy chain 之后，会继续跑 `check_reference_redlines.py`，直接拦截未入账/已漂移/已逾期的结构性债务、未登记的绕过白名单、无主 TODO、空异常处理、异常上下文未绑定/未使用、`OSError` / `json.JSONDecodeError` / `FileExistsError` / `KeyError` / `RuntimeError` / `SyntaxError` 上下文丢失，以及高风险 I/O/JSON 直接静默降级
