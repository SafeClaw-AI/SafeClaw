...
- 在这条 ledger policy chain 之后，会继续跑 `check_reference_redlines.py`，直接拦截无主 TODO、空异常处理、异常上下文未绑定/未使用、`OSError` / `json.JSONDecodeError` / `FileExistsError` / `KeyError` / `RuntimeError` 上下文丢失，以及高风险 I/O/JSON 直接静默降级
