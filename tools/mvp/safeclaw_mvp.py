    ...
    if has_flag(args, "--json"):
        ...
        if preflight_payload is not None:
            ...
            if not bool(preflight_payload.get("allowed")):
                ...
                return emit_json_error(local_action, "failed step=preflight", exit_code=1, code="preflight-blocked", reason=str(preflight_payload.get("reason") or "").strip() or None, summary=str(details.get("preflight_summary") or "").strip() or None, requested_action=str(details.get("preflight_requested_action") or preflight_payload.get("requested_action") or "").strip() or None, details=details)
        try:
...
def run_sequence_json(
    ...
    if preflight_payload is not None:
        ...
        if not bool(preflight_payload.get("allowed")):
            details = build_preflight_blocked_details(preflight_payload, step_results)
            return emit_json_error(name, "failed step=preflight", exit_code=1, code="preflight-blocked", reason=str(details.get("preflight_reason") or preflight_payload.get("reason") or "").strip() or None, summary=str(details.get("preflight_summary") or "").strip() or None, requested_action=str(details.get("preflight_requested_action") or preflight_payload.get("requested_action") or "").strip() or None, details=details)
    for step in steps:
        try:
