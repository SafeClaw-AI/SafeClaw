...
- `service-status`: queue / lease / task snapshot summary for the selected db, including recent task `scope` / `write` / `doctor_bypass` visibility, latest lease freshness, a `next_action` hint, and a copyable `next_command`
