...
- `service-status`: queue / lease / task snapshot summary for the selected db, including recent task `scope` / `write` / `doctor_bypass` visibility, latest lease freshness, active-lease wait timing, a `next_action` hint, a copyable `next_command`, a short `next_reason`, and the current `next_blocker`
...
- `service-status` successful `--json` returns structured fields like `queue`, `workers`, `effects`, `probes`, and `recent_tasks`; each recent task now also includes `target_scope`, `requires_write`, `doctor_bypass`, `lease_state`, `lease_owner_id`, latest lease snapshot fields, `lease_remaining_ms` for active leases, `next_action` (`ok` / `retry` / `recover` / `inspect`), a copyable `next_command`, a short `next_reason`, and `next_blocker` (`none` / `active_lease` / `manual_review_needed`).
