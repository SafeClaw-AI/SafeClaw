...
- `service-status`: queue / lease / task snapshot summary for the selected db, including heartbeat / coordination summaries plus recent task `scope` / `write` / `doctor_bypass` visibility, same-scope peer visibility (`scope_peer_count` / `scope_active_peer_count` / `scope_active_peer_task_id`), current `permission_tier` / `permission_policy` / `permission_reason`, latest lease freshness, active-lease wait timing, a `next_action` hint, a copyable `next_command`, a short `next_reason`, the current `next_blocker`, task-level `coordination_status` / `coordination_reason` / `coordination_summary`, and a one-line `next_summary`
...
- `preflight`: checks whether a target action remains allowed in the current local-only MVP entry; `--json` also returns `tier`, `decision`, `offline_ready`, optional `target_scope` / `requires_write` / `doctor_bypass`, `permission_context_source`, `permission_context_applied`, `permission_enforced`, `action_allowed` / `action_decision` / `action_reason`, `permission_tier` / `permission_policy` / `permission_reason`, and current runtime/provider snapshots
...
- `service-status` successful `--json` returns structured fields like `queue`, `workers`, `effects`, `probes`, top-level `heartbeat`, top-level `coordination`, and `recent_tasks`; each recent task now also includes `target_scope`, `requires_write`, `doctor_bypass`, `permission_tier`, `permission_policy`, `permission_reason`, `lease_state`, `lease_owner_id`, latest lease snapshot fields, `lease_remaining_ms` for active leases, same-scope peer visibility (`scope_peer_count` / `scope_active_peer_count` / `scope_active_peer_task_id`), `next_action` (`ok` / `retry` / `recover` / `inspect`), a copyable `next_command`, a short `next_reason`, `next_blocker` (`none` / `active_lease` / `manual_review_needed`), task-level `coordination_status` / `coordination_reason` / `coordination_summary`, and a concise `next_summary`.
