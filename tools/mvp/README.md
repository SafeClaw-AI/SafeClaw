...
- 对因 wrapper preflight 被阻断的组合动作，顶层 `error.code` 现稳定返回 `preflight-blocked`，`error.details.code` 仍保留同值。
