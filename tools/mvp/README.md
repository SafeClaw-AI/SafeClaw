...
- Optional explicit offline gate: `preflight --action service-run` before executing the local flow; add `--scope demo.workspace` / `--write` / `--doctor-bypass` when you want an upfront permission decision, and add `--enforce-permission` when scripts should fail closed on `confirm` / `deny`.
...
- `preflight`: explicit gate for whether a target action stays allowed in the current local-only MVP entry; known local actions allow, unknown actions default deny, optional `--scope` / `--write` / `--doctor-bypass` surface `permission_tier` / `permission_policy` / `permission_reason` before execution, and `--enforce-permission` turns that surfaced permission decision into a fail-closed gate
...
- `preflight`: checks whether a target action remains allowed in the current local-only MVP entry; `--json` also returns `tier`, `decision`, `offline_ready`, optional `target_scope` / `requires_write` / `doctor_bypass`, `permission_context_applied`, `permission_enforced`, `action_allowed` / `action_decision` / `action_reason`, `permission_tier` / `permission_policy` / `permission_reason`, and current runtime/provider snapshots
...
- `service-status` successful `--json` returns structured fields like `queue`, `workers`, `effects`, `probes`, and `recent_tasks`; each recent task now also includes `target_scope`, `requires_write`, `doctor_bypass`, `permission_tier`, `permission_policy`, `permission_reason`, `lease_state`, `lease_owner_id`, latest lease snapshot fields, `lease_remaining_ms` for active leases, `next_action` (`ok` / `retry` / `recover` / `inspect`), a copyable `next_command`, a short `next_reason`, `next_blocker` (`none` / `active_lease` / `manual_review_needed`), and a concise `next_summary`.
