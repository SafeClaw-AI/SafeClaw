...
- `service-status`: queue / lease / task snapshot summary for the selected db, including recent task `scope` / `write` / `doctor_bypass` visibility, latest lease freshness, and a `next_action` hint
...
- `service-status` successful `--json` returns structured fields like `queue`, `workers`, `effects`, `probes`, and `recent_tasks`; each recent task now also includes `target_scope`, `requires_write`, `doctor_bypass`, `lease_state`, `lease_owner_id`, latest lease snapshot fields, and `next_action` (`ok` / `retry` / `recover` / `inspect`).
