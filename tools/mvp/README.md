...
- Optional explicit offline gate: `preflight --action service-run` before executing the local flow; add `--scope demo.workspace` / `--write` / `--doctor-bypass` when you want an upfront permission decision.
...
- `preflight`: explicit gate for whether a target action stays allowed in the current local-only MVP entry; known local actions allow, unknown actions default deny, and optional `--scope` / `--write` / `--doctor-bypass` surface `permission_tier` / `permission_policy` / `permission_reason` before execution
...
- `preflight`: checks whether a target action remains allowed in the current local-only MVP entry; `--json` also returns `tier`, `decision`, `offline_ready`, optional `target_scope` / `requires_write` / `doctor_bypass`, `permission_context_applied`, `permission_tier` / `permission_policy` / `permission_reason`, and current runtime/provider snapshots
