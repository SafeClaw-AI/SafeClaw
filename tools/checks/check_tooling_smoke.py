...
def assert_preflight_json_result(
    ...
    *,
    ...
    if result.get("requested_action") != expected_requested_action:
    ...
    elif result.get("permission_context_applied") is not expected_permission_context_applied:
        errors.append(f"{name} missing permission_context_applied={expected_permission_context_applied}")
        ...
        errors.append(f"{name} missing permission_enforced={expected_permission_enforced}")
...
def assert_service_status_json_result(
    ...
    *,
    ...
    elif expected_permission_tier is not None and recent_tasks[0].get("permission_tier") != expected_permission_tier:
        errors.append(f"{name} missing recent permission_tier={expected_permission_tier}")
    elif expected_permission_policy is not None and recent_tasks[0].get("permission_policy") != expected_permission_policy:
        errors.append(f"{name} missing recent permission_policy={expected_permission_policy}")
    elif expected_permission_reason is not None and recent_tasks[0].get("permission_reason") != expected_permission_reason:
    ...
    elif expected_next_reason is not None and recent_tasks[0].get("next_reason") != expected_next_reason:
        errors.append(f"{name} missing recent next_reason={expected_next_reason}")
    elif expected_next_blocker is not None and recent_tasks[0].get("next_blocker") != expected_next_blocker:
...
def assert_service_reconcile_json_result(
    ...
    *,
    ...
    if reconcile_payload.get("saved_session") is not None:
        ...
        return
    if not isinstance(nested_remembered_session, dict) or nested_remembered_session.get("task_id") != expected_task_id:
        errors.append(f"{name} nested reconcile missing remembered_session {expected_task_id}")
        return
    assert_step_source_hints(
        steps[:2],
        errors,
    ...
    *,
    ...
    if expected_top_level_error_error_code is not None:
        if error.get("error_code") != expected_top_level_error_error_code:
            errors.append(f"{name} missing error.error_code={expected_top_level_error_error_code}")
            return
    ...
    if expected_top_level_error_degradation_mode is not None:
        if error.get("degradation_mode") != expected_top_level_error_degradation_mode:
            errors.append(f"{name} missing error.degradation_mode={expected_top_level_error_degradation_mode}")
            return
    if expected_top_level_error_requires_model is not None:
        if error.get("requires_model") is not expected_top_level_error_requires_model:
            errors.append(f"{name} missing error.requires_model={expected_top_level_error_requires_model}")
            return
    if expected_top_level_error_requires_sidecar is not None:
        if error.get("requires_sidecar") is not expected_top_level_error_requires_sidecar:
            errors.append(f"{name} missing error.requires_sidecar={expected_top_level_error_requires_sidecar}")
            return
...
def collect_errors() -> list[str]:
    ...
    result = assert_command_json_result(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "preflight", "--action", "service-status", "--scope", "demo.workspace", "--write", "--json"],
        errors,
    ...
    wrapper_preflight_scope_enforced = subprocess.run(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "preflight", "--action", "service-status", "--scope", "demo.workspace", "--write", "--enforce-permission"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    ...
    payload = load_json_payload(
        run_wrapper_command([PYTHON, "tools/mvp/safeclaw_mvp.py", "preflight", "--action", "service-status", "--scope", "demo.workspace", "--write", "--enforce-permission", "--json"]),
        errors,
    ...
    if payload is not None:
        if payload.get("ok") is not False or payload.get("action") != "preflight":
            ...
            candidate = payload.get("result")
            if not isinstance(candidate, dict):
                errors.append("mvp-wrapper-preflight-scope-enforced-json missing result payload")
            else:
                result = candidate
    assert_preflight_json_result(
        result,
    ...
    payload = load_json_payload(
        run_wrapper_command([PYTHON, "tools/mvp/safeclaw_mvp.py", "preflight", "--action", "service-run", "--enforce-permission", "--json"]),
        errors,
    ...
    if payload is not None:
        if payload.get("ok") is not False or payload.get("action") != "preflight":
            ...
            candidate = payload.get("result")
            if not isinstance(candidate, dict):
                errors.append("mvp-wrapper-preflight-enforce-without-context-json missing result payload")
            else:
                result = candidate
    assert_preflight_json_result(
        result,
    ...
    result = assert_command_json_result(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "preflight", "--action", "service-run", "--scope", "demo.workspace", "--doctor-bypass", "--enforce-permission", "--json"],
        errors,
    ...
    if result is not None:
        ...
        if queue.get("queued") != 0:
            ...
            elif not isinstance(next_summary, str) or not next_summary.startswith("wait:remaining_ms="):
    ...
    wrapper_service_run_output = (wrapper_service_run.stdout or "") + (wrapper_service_run.stderr or "")
    if wrapper_service_run.returncode != 0:
        errors.append(f"mvp-wrapper-service-run failed: exit={wrapper_service_run.returncode}")
    ...
    elif "[mvp] accepted task => task=task-wrapper-service-run effect=effect-task-wrapper-service-run" not in wrapper_service_run_output:
        errors.append("mvp-wrapper-service-run missing run acceptance output")
    elif "[mvp-wrapper] service-status => db=target/mvp/service-run.db limit=1 source=flag" not in wrapper_service_run_output:
    ...
    elif "[mvp-wrapper] service-run => preflight" not in wrapper_service_run_preflight_ai_reason_output:
        errors.append("mvp-wrapper-service-run-preflight-ai-reason missing preflight step marker")
    elif "[mvp-wrapper] preflight => action=ai-reason known=true class=ai-action tier=TIER_2 writes_state=false target_scope=scope:target/mvp/service-run-preflight-ai.txt requires_write=true doctor_bypass=false perm_ctx=true perm_ctx_src=prepared-action enforce_perm=false perm=confirm perm_tier=TIER_1 perm_reason=write_scope_requires_confirmation decision=deny allowed=false offline_ready=false requires_model=true requires_sidecar=true degradation=provider_unavailable reason=ERR_AI_PROVIDER_UNAVAILABLE error_code=ERR_AI_PROVIDER_UNAVAILABLE" not in wrapper_service_run_preflight_ai_reason_output:
        errors.append("mvp-wrapper-service-run-preflight-ai-reason missing provider-unavailable preflight summary")
    elif "[mvp-wrapper] service-run => failed step=preflight exit=1" not in wrapper_service_run_preflight_ai_reason_output:
        errors.append("mvp-wrapper-service-run-preflight-ai-reason missing failed preflight marker")
    ...
    if isinstance(details, dict):
        ...
        if not isinstance(preflight_payload, dict):
            errors.append("mvp-wrapper-service-run-preflight-ai-json missing preflight payload")
        else:
            assert_preflight_json_result(
                preflight_payload,
                errors,
        ...
        if not isinstance(steps, list) or len(steps) != 1 or not isinstance(steps[0], dict):
            errors.append("mvp-wrapper-service-run-preflight-ai-json missing isolated preflight step")
        elif steps[0].get("action") != "preflight":
            errors.append("mvp-wrapper-service-run-preflight-ai-json missing preflight step action")
    ...
    if result is not None:
        ...
        if queue.get("expired") != 1:
            errors.append("mvp-wrapper-service-retry-status-before-json missing queue.expired=1")
        elif queue.get("completed") != 0:
            errors.append("mvp-wrapper-service-retry-status-before-json missing queue.completed=0")
        elif workers.get("failed") != 1:
            errors.append("mvp-wrapper-service-retry-status-before-json missing workers.failed=1")
        elif effects.get("prepared") != 1:
            errors.append("mvp-wrapper-service-retry-status-before-json missing effects.prepared=1")
        elif probes.get("none") != 1:
            errors.append("mvp-wrapper-service-retry-status-before-json missing probes.none=1")
        elif not isinstance(recent_tasks, list) or not recent_tasks:
            errors.append("mvp-wrapper-service-retry-status-before-json missing recent task")
        elif recent_tasks[0].get("permission_tier") != "TIER_1":
            errors.append("mvp-wrapper-service-retry-status-before-json missing permission_tier=TIER_1")
        elif recent_tasks[0].get("permission_policy") != "confirm":
            errors.append("mvp-wrapper-service-retry-status-before-json missing permission_policy=confirm")
        elif recent_tasks[0].get("permission_reason") != "write_scope_requires_confirmation":
            errors.append("mvp-wrapper-service-retry-status-before-json missing permission_reason=write_scope_requires_confirmation")
        elif recent_tasks[0].get("next_action") != "retry":
            ...
            errors.append("mvp-wrapper-service-retry-status-before-json missing next_reason=failed_state_ready_for_retry")
    ...
    wrapper_service_retry_output = (wrapper_service_retry.stdout or "") + (wrapper_service_retry.stderr or "")
    if wrapper_service_retry.returncode != 0:
        errors.append(f"mvp-wrapper-service-retry failed: exit={wrapper_service_retry.returncode}")
    ...
    elif "[mvp-wrapper] service-retry => service-status" not in wrapper_service_retry_output:
        errors.append("mvp-wrapper-service-retry missing service-status step marker")
    elif "[mvp] retry result => worker=Succeeded, effect=Executed, completed=true" not in wrapper_service_retry_output:
        errors.append("mvp-wrapper-service-retry missing retry success output")
    elif "[mvp-wrapper] service-status => db=target/mvp/service-retry.db limit=1 source=flag" not in wrapper_service_retry_output:
    ...
    assert_command_json_error(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "service-retry", "--limit", "bogus", "--json"],
        errors,
    ...
    assert_command_json_error(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "service-retry", "--db", "target/mvp/service-retry-missing.db", "--json"],
        errors,
    ...
    if result is not None:
        ...
        recent_tasks = result.get("recent_tasks") or []
        if queue.get("expired") != 1:
            errors.append("mvp-wrapper-service-recover-status-before-json missing queue.expired=1")
        elif queue.get("completed") != 0:
            errors.append("mvp-wrapper-service-recover-status-before-json missing queue.completed=0")
        elif workers.get("uncertain") != 1:
            errors.append("mvp-wrapper-service-recover-status-before-json missing workers.uncertain=1")
        elif effects.get("uncertain") != 1:
            errors.append("mvp-wrapper-service-recover-status-before-json missing effects.uncertain=1")
        elif probes.get("probe_pending") != 1:
            errors.append("mvp-wrapper-service-recover-status-before-json missing probes.probe_pending=1")
        elif not isinstance(recent_tasks, list) or not recent_tasks:
            errors.append("mvp-wrapper-service-recover-status-before-json missing recent task")
        elif recent_tasks[0].get("permission_tier") != "TIER_1":
            errors.append("mvp-wrapper-service-recover-status-before-json missing permission_tier=TIER_1")
        elif recent_tasks[0].get("permission_policy") != "confirm":
            errors.append("mvp-wrapper-service-recover-status-before-json missing permission_policy=confirm")
        elif recent_tasks[0].get("permission_reason") != "write_scope_requires_confirmation":
            errors.append("mvp-wrapper-service-recover-status-before-json missing permission_reason=write_scope_requires_confirmation")
            ...
            errors.append("mvp-wrapper-service-recover-status-before-json missing next_reason=uncertain_state_ready_for_recover")
        elif recent_tasks[0].get("next_blocker") != "none":
    ...
    wrapper_service_recover_output = (wrapper_service_recover.stdout or "") + (wrapper_service_recover.stderr or "")
    if wrapper_service_recover.returncode != 0:
        errors.append(f"mvp-wrapper-service-recover failed: exit={wrapper_service_recover.returncode}")
    elif "[mvp-wrapper] service-recover => recover" not in wrapper_service_recover_output:
        errors.append("mvp-wrapper-service-recover missing recover step marker")
    elif "[mvp-wrapper] service-recover => service-status" not in wrapper_service_recover_output:
        errors.append("mvp-wrapper-service-recover missing service-status step marker")
    elif "[mvp] recover result => from=Uncertain, worker=Succeeded, effect=Executed, completed=true" not in wrapper_service_recover_output:
        errors.append("mvp-wrapper-service-recover missing recover success output")
    elif "[mvp-wrapper] service-status => db=target/mvp/service-recover.db limit=1 source=flag" not in wrapper_service_recover_output:
        errors.append("mvp-wrapper-service-recover missing service-status output")
    elif "[mvp-wrapper] service workers => succeeded=1" not in wrapper_service_recover_output:
    ...
    assert_command_json_error(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "service-recover", "--limit", "bogus", "--json"],
        errors,
    ...
    assert_command_json_error(
        [PYTHON, "tools/mvp/safeclaw_mvp.py", "service-recover", "--db", "target/mvp/service-recover-missing.db", "--json"],
        errors,
    ...
    assert_command_json_error(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", "tools\mvp\safeclaw_mvp.ps1", "status", "--bogus", "--json"],
        errors,
    ...
    assert_command_json_error(
        ["cmd", "/c", "tools\mvp\safeclaw_mvp.cmd", "resume", "--db", "target/mvp/resume-missing-cmd.db", "--json"],
        errors,
    ...
    assert_command_json_error(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", "tools\mvp\safeclaw_mvp.ps1", "reconcile", "--db", "target/mvp/reconcile-missing-ps1.db", "--decision", "executed", "--json"],
        errors,
    ...
    assert_command_json_error(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", "tools\mvp\safeclaw_mvp.ps1", "retry", "--bogus", "--json"],
        errors,
    ...
    assert_command_json_error(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", "tools\mvp\safeclaw_mvp.ps1", "session", "--bogus", "--json"],
        errors,
    ...
    wrapper_service_status_reconcile_output = (wrapper_service_status_reconcile.stdout or "") + (wrapper_service_status_reconcile.stderr or "")
    if wrapper_service_status_reconcile.returncode != 0:
        errors.append(f"mvp-wrapper-service-reconcile-status-before failed: exit={wrapper_service_status_reconcile.returncode}")
    elif "[mvp-wrapper] service coordination => status=quarantined reason=self_executed_assumed_scope_quarantine summary=reconcile_self_before_scope_write task=task-wrapper-service-reconcile-json" not in wrapper_service_status_reconcile_output:
    ...
    if result is not None:
        ...
        recent_tasks = result.get("recent_tasks") or []
        current_session = result.get("current_session") or {}
        if not isinstance(current_session, dict) or current_session.get("task_id") != "task-wrapper-service-reconcile-json":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing current_session task-wrapper-service-reconcile-json")
        elif coordination.get("status") != "quarantined":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.status=quarantined")
        elif coordination.get("reason") != "self_executed_assumed_scope_quarantine":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.reason=self_executed_assumed_scope_quarantine")
        elif coordination.get("summary") != "reconcile_self_before_scope_write":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.summary=reconcile_self_before_scope_write")
        elif coordination.get("scope_quarantine_active") is not True:
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.scope_quarantine_active=true")
        elif coordination.get("scope_quarantine_source") != "self":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.scope_quarantine_source=self")
        elif coordination.get("scope_quarantine_task_id") != "task-wrapper-service-reconcile-json":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.scope_quarantine_task_id=task-wrapper-service-reconcile-json")
        elif coordination.get("next_task_id") != "task-wrapper-service-reconcile-json":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing coordination.next_task_id=task-wrapper-service-reconcile-json")
        elif not isinstance(recent_tasks, list) or not recent_tasks:
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing recent task")
        elif recent_tasks[0].get("task_id") != "task-wrapper-service-reconcile-json":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing recent task task-wrapper-service-reconcile-json")
        elif recent_tasks[0].get("next_action") != "inspect":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing next_action=inspect")
        elif recent_tasks[0].get("next_reason") != "executed_assumed_requires_reconcile":
            errors.append("mvp-wrapper-service-reconcile-status-before-json missing next_reason=executed_assumed_requires_reconcile")
        elif recent_tasks[0].get("next_blocker") != "scope_quarantine":
    ...
    if isinstance(details, dict):
        preflight_payload = details.get("preflight")
        if not isinstance(preflight_payload, dict):
            errors.append("mvp-wrapper-demo-preflight-ai-json missing preflight payload")
        else:
            assert_preflight_json_result(
                preflight_payload,
                errors,
        ...
        steps = details.get("steps") or []
        if not isinstance(steps, list) or len(steps) != 1 or not isinstance(steps[0], dict):
            errors.append("mvp-wrapper-demo-preflight-ai-json missing isolated preflight step")
        elif steps[0].get("action") != "preflight":
            errors.append("mvp-wrapper-demo-preflight-ai-json missing preflight step action")
    ...
    assert_command_json_error(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", "tools\mvp\safeclaw_mvp.ps1", "demo", "--bogus", "--json"],
        errors,
    ...
    assert_command_json_error(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", "tools\mvp\safeclaw_mvp.ps1", "retry-demo", "--bogus", "--json"],
        errors,
