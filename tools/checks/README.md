...
- `check_reference_redlines.py`：把 `docs/reference/01` 中已机器化的红线直接落成代码门禁；当前除了拦无主 TODO、空异常处理、多异常 `except` 必须绑定并真正使用 `as error`、`OSError` / `json.JSONDecodeError` / `FileExistsError` / `KeyError` / `RuntimeError` / `SyntaxError` 必须绑定并保留上下文，以及高风险 `OSError/json.JSONDecodeError` 不能直接静默降级为 `None/False` 之外，也会从 `docs/reference/01-反屎山工程规范.md` 读取体量/复杂度阈值，结合 `docs/reference/02-结构性债务台账.md` 校验结构性债务是否已入账、未漂移、未逾期，并结合 `docs/reference/03-绕过白名单.md` 校验 `# noqa` / warning filter 等绕过是否已登记
...
- 这条 ledger policy chain 之后，会继续跑 `check_reference_redlines.py`，把结构性债务台账对齐、绕过白名单对齐、无主 TODO、空异常处理、异常上下文绑定/使用、`OSError` / `json.JSONDecodeError` / `FileExistsError` / `KeyError` / `RuntimeError` / `SyntaxError` 上下文保留，以及高风险 I/O/JSON 静默降级挡在 `Naming lint` 和 `Contract tests` 之前
