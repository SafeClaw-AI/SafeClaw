...
- `check_reference_redlines.py`：把 `docs/reference/01` 中已机器化的红线直接落成代码门禁；当前先拦无主 TODO、空异常处理、多异常 `except` 必须绑定并真正使用 `as error`、`OSError` / `json.JSONDecodeError` / `FileExistsError` 必须绑定并保留上下文，以及高风险 `OSError/json.JSONDecodeError` 不能直接静默降级为 `None/False`
...
- 这条 ledger policy chain 之后，会继续跑 `check_reference_redlines.py`，把无主 TODO、空异常处理、异常上下文绑定/使用、`OSError` / `json.JSONDecodeError` / `FileExistsError` 上下文保留，以及高风险 I/O/JSON 静默降级挡在 `Naming lint` 和 `Contract tests` 之前
