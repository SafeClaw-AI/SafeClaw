...
_RETRY_DEMO_TEXT_LABEL = "mvp-wrapper-retry-demo-fail"
_RETRY_DEMO_FAIL_SUBSTRING = "[mvp-wrapper] retry-demo => failed step=seed-failed exit=2"
_RETRY_DEMO_MISSING_OUTPUT_ERROR = "mvp-wrapper-retry-demo-fail 输出缺少组合动作失败承接提示"


@dataclass(frozen=True)
class WrapperRetryDemoFailureContext:
    repo_root: Path
...
def _append_retry_demo_fail_text_errors(
    errors: list[str],
    ctx: WrapperRetryDemoFailureContext,
) -> None:
    ctx.assert_command_failure_output(
        _ps1_command("retry-demo", "--bogus"),
        errors,
        "mvp-wrapper-ps1-retry-demo-fail",
        expected_exit=2,
        expected_substring=_RETRY_DEMO_FAIL_SUBSTRING,
        missing_output_label="mvp-wrapper-ps1-retry-demo-fail missing failed step marker",
    )

    wrapper_retry_demo_fail = ctx.subprocess_module.run(
        _py_command(ctx.python_executable, "retry-demo", "--bogus"),
        cwd=ctx.repo_root,
        capture_output=True,
        text=True,
    )
    output = _collect_output(wrapper_retry_demo_fail)
    if wrapper_retry_demo_fail.returncode != 2:
        errors.append(
            f"{_RETRY_DEMO_TEXT_LABEL} 执行失败: exit={wrapper_retry_demo_fail.returncode}"
        )
    elif _RETRY_DEMO_FAIL_SUBSTRING not in output:
        errors.append(_RETRY_DEMO_MISSING_OUTPUT_ERROR)


def _append_retry_demo_fail_json_errors(
    errors: list[str],
    ctx: WrapperRetryDemoFailureContext,
) -> None:
    ctx.assert_command_json_error(
        _cmd_command("retry-demo", "--bogus", "--json"),
        errors,
        ...
        expected_remembered_session_task_id=_RETRY_DEMO_TASK_ID,
    ...
    ctx.assert_command_json_error(
        _ps1_command("retry-demo", "--bogus", "--json"),
        errors,
    ...
    ctx.assert_command_json_error(
        _py_command(ctx.python_executable, "retry-demo", "--bogus", "--json"),
        errors,
...
def append_wrapper_retry_demo_failure_errors(
    errors: list[str],
    *,
    repo_root: Path,
    python_executable: str,
    ...
    _append_retry_demo_fail_text_errors(errors, ctx)
    _append_retry_demo_fail_json_errors(errors, ctx)
