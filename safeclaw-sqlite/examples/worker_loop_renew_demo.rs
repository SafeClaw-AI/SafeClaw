...
fn main() -> Result<(), String> {
    ...
    let output_bytes = b"safeclaw worker renew demo\n";
    ...
    let renewed = into_demo(first_worker.renew_claim_lease(&failed.claim, 20))?;
    println!(
        "[demo] renewed lease => id={} owner={} fence={} expires_at_ms={}",
        renewed.lease.lease_id,
        renewed.lease.owner_id,
        renewed.lease.fencing_token,
        renewed.lease.expires_at_ms
    );
    print_snapshot("after-renew", first_worker.queue_snapshot());
    ...
    let retried = into_demo(retry_worker.claim_and_retry_failed_once(
        "worker-b",
        46,
        "effect-worker-loop-renew-demo",
        PreflightDecision::Permit,
        |claim, runtime| {
            println!(
                "[demo] reclaim after renewed expiry => task={} lease={} fence={} state={:?}",
                claim.task.task_id,
    ...
    .expect("renewed lease must still expire eventually");
    ...
    .expect("persisted renewed runtime must reload");
    println!(
...
fn demo_effect(claim: &OrchestratorClaim) -> EffectRecord {
    EffectRecord::new(
        ...
        "trace-worker-loop-renew-demo",
        "intent-worker-loop-renew-demo",
        EffectActor::Worker,
...
fn sandbox_write_command(output_path: &Path, output_bytes: &[u8]) -> SandboxCommand {
    if cfg!(windows) {
        ...
        let text = String::from_utf8(output_bytes.to_vec())
            .expect("worker renew demo bytes must remain utf-8");
        SandboxCommand::new(
            "sh",
            [
                "-c",
...
impl DemoArtifacts {
    fn new(workspace: &Path) -> Result<Self, String> {
        ...
        let root = workspace
            .join("target")
            .join(format!("worker-loop-renew-demo-{}-{unique}", process::id()));
        fs::create_dir_all(&root).map_err(|error| error.to_string())?;
        Ok(Self {
            output_path: root.join("worker-loop-renew-output.txt"),
            db_path: root.join("worker-loop-renew.db"),
            root,
        })
    }
