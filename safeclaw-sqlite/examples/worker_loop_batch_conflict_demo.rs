...
fn sandbox_write_command(output_path: &Path, output_bytes: &[u8]) -> SandboxCommand {
    if cfg!(windows) {
        ...
        let text = String::from_utf8(output_bytes.to_vec())
            .expect("worker batch conflict demo bytes must remain utf-8");
        SandboxCommand::new(
            "sh",
            [
                "-c",
...
impl DemoArtifacts {
    fn new(workspace: &Path) -> Result<Self, String> {
        ...
        let root = workspace
            .join("target")
            .join(format!("worker-loop-batch-conflict-demo-{}-{unique}", process::id()));
        fs::create_dir_all(&root).map_err(|error| error.to_string())?;
        Ok(Self {
            db_path: root.join("worker-loop-batch-conflict.db"),
            root,
        })
    }
