...
fn main() -> Result<(), String> {
    ...
    let output_bytes = b"safeclaw worker network demo\n";
    ...
    println!("[demo] target: {}", server.target());
...
fn demo_effect(claim: &OrchestratorClaim) -> EffectRecord {
    EffectRecord::new(
        ...
        "trace-worker-loop-network-demo",
        "intent-worker-loop-network-demo",
        EffectActor::Worker,
...
fn sandbox_write_then_timeout_command(
    ...
    if cfg!(windows) {
        ...
        let text = String::from_utf8(output_bytes.to_vec())
            .expect("worker network demo bytes must remain utf-8");
        SandboxCommand::new(
            "sh",
            [
                "-c",
...
impl DemoArtifacts {
    fn new(workspace: &Path) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            db_path: root.join("worker-loop-network.db"),
            root,
        })
    }
