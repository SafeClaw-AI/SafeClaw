...
fn main() -> Result<(), String> {
    ...
    let fourth = into_demo(orchestrator.claim_next("orch-d", 4))?
        .expect("second same-scope write must unblock after first write completes");
    println!(
        "[demo] second write claim => task={} lease={} fence={} owner={}",
        fourth.task.task_id,
        fourth.lease.lease_id,
        fourth.lease.fencing_token,
        fourth.lease.owner_id
    );
    assert!(fourth.task.intent.requires_write);

    let snapshot = orchestrator.queue_snapshot();
    print_snapshot("after-second-write-claim", snapshot.clone());
