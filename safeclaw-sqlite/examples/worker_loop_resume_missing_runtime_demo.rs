...
fn main() -> Result<(), String> {
    ...
    let error = worker
        .claim_and_resume_persisted_once(
            "worker-a",
            0,
            "effect-worker-loop-missing-resume-demo",
            |_, _| unreachable!(),
        )
    ...
    match &error {
        WorkerLoopError::PersistedRuntimeMissing { task_id, effect_id } => {
            assert_eq!(task_id, "task-worker-loop-missing-resume-demo");
            assert_eq!(effect_id, "effect-worker-loop-missing-resume-demo");
            println!(
...
impl DemoArtifacts {
    fn new(workspace: &Path) -> Result<Self, String> {
        ...
        let root = workspace.join("target").join(format!(
            "worker-loop-missing-resume-demo-{}-{unique}",
            process::id()
        ));
        fs::create_dir_all(&root).map_err(|error| error.to_string())?;
        Ok(Self {
            output_path: root.join("worker-loop-missing-resume-output.txt"),
            db_path: root.join("worker-loop-missing-resume.db"),
            root,
        })
    }
