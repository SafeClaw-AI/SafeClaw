    ...
    let released = into_demo(release_worker.claim_and_drive_once(
        ...
        |claim| {
            ...
            Ok((
                InMemoryTaskRuntime::new(demo_effect(
                    claim,
                    "effect-worker-loop-retry-release-shared",
                    "trace-worker-loop-retry-release-shared",
                    "intent-worker-loop-retry-release-shared",
                )),
                sandbox_write_command(&shared_output, b"safeclaw shared after retry release\n"),
            ))
        },
    ))?
...
fn sandbox_write_command(output_path: &Path, output_bytes: &[u8]) -> SandboxCommand {
    if cfg!(windows) {
        ...
        let text = String::from_utf8(output_bytes.to_vec())
            .expect("worker retry release demo bytes must remain utf-8");
        SandboxCommand::new(
            "sh",
            [
                "-c",
...
impl DemoArtifacts {
    fn new(workspace: &Path) -> Result<Self, String> {
        ...
        let root = workspace
            .join("target")
            .join(format!("worker-loop-retry-release-demo-{}-{unique}", process::id()));
        fs::create_dir_all(&root).map_err(|error| error.to_string())?;
        Ok(Self {
            db_path: root.join("worker-loop-retry-release.db"),
            root,
        })
    }
