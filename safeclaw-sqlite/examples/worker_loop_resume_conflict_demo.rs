...
fn main() -> Result<(), String> {
        ...
        .claim_and_drive_once("worker-b", 1, PreflightDecision::Permit, |claim| {
            ...
            Ok((
                InMemoryTaskRuntime::new(demo_effect(
                    claim,
                    "effect-worker-loop-resume-conflict-demo",
                    "trace-worker-loop-resume-conflict-demo",
                    "intent-worker-loop-resume-conflict-demo",
                )),
    ...
    .expect("expired other-scope runtime must be reclaimable");
...
fn sandbox_write_command(output_path: &Path, output_bytes: &[u8]) -> SandboxCommand {
    if cfg!(windows) {
        ...
        let text = String::from_utf8(output_bytes.to_vec())
            .expect("worker resume conflict demo bytes must remain utf-8");
        SandboxCommand::new(
            "sh",
            [
                "-c",
...
impl DemoArtifacts {
    fn new(workspace: &Path) -> Result<Self, String> {
        ...
        let root = workspace
            .join("target")
            .join(format!("worker-loop-resume-conflict-demo-{}-{unique}", process::id()));
        fs::create_dir_all(&root).map_err(|error| error.to_string())?;
        Ok(Self {
            db_path: root.join("worker-loop-resume-conflict.db"),
            root,
        })
    }
